...
RadAgent forces every output to be grounded in four independent evidence layers before any natural-language report is written:
...
2. **Agentic RAG retrieval** — the VLM evaluates retrieval sufficiency for each above-threshold finding and iteratively refines queries (up to 2 iterations) when passages are weak. A multilingual FAISS index (1,078 chunks from Wikipedia + StatPearls) returns clinically relevant passages with source URLs.
...
4. **Self-audit** — before finalizing, the VLM audits its own report against specialist findings and retrieved evidence to flag inconsistencies.

Only after all four layers are validated does Qwen2.5-VL-7B-Instruct (running on AMD MI300X via vLLM-ROCm) compose the structured report. **Every claim it makes carries a numbered citation pointing to a real retrieved passage with a real URL.** No floating assertions. No confabulated references.
...
### v0.4 highlights — agentic RAG with VLM-controlled retrieval

**New in feature/agentic-rag branch:** The VLM now controls its own evidence gathering. Instead of passive retrieval, the system:

1. **Evaluates retrieval sufficiency** — after initial RAG retrieval, the VLM assesses whether passages are adequate for clinical grounding
2. **Refines queries iteratively** — if passages are weak, the VLM generates a refined query and retrieves again (up to 2 iterations)
3. **Self-audits the report** — before finalizing, the VLM audits its own report against specialist findings and retrieved evidence, flagging any inconsistencies

The dashboard now shows the **agentic** and **audit** stages in the pipeline with real-time progress:

![Agentic RAG pipeline with audit stage](docs/demo_v3/02_pipeline_evidence.png)

Reports include an audit badge indicating grounding status: **"Audited ✓ Grounded"** or **"Audited ⚠ N issues"**.
...
#### Agentic RAG (v0.4 — feature/agentic-rag branch)
...
| Specialist forward (TTA) | 22 ms | 1,762 ms* |
| Findings layer | 0.1 ms | 0.2 ms |
| **Agentic RAG** (with query refinement) | **500 ms** | **2,100 ms** |
| Grad-CAM | < 50 ms | < 100 ms |
| VLM (Qwen2.5-VL-7B) | 5,535 ms | 5,640 ms |
| **Self-audit** | **3,339 ms** | **4,200 ms** |
| **Total** | **~9,400 ms** | **~13,700 ms** |

\* p95 outliers come from cold-start cases (first request hits cold caches).

**Agentic overhead:** +1.5s for iterative retrieval evaluation, +3.3s for self-audit. The VLM makes 2-3 additional inference calls per case to control its own evidence gathering and validate output quality.
...
│   └── bench_mi300x.py            # MI300X latency benchmark (agentic RAG support)
├── infra/
...
**Baseline (passive RAG):**
```bash
python scripts/bench_mi300x.py --n 50 --output-dir runs/bench/ --no-agentic
```
...
The benchmark script logs per-finding agentic decisions and self-audit results to stdout:
```
...
- **The VLM is general-purpose, not medically fine-tuned.** Qwen2.5-VL-7B can still produce subtly wrong terminology even when grounded. The structured prompt + citations + self-audit reduce hallucination but do not eliminate it.
- **Agentic RAG adds latency.** Query refinement and self-audit add ~4.8s per case. For time-critical scenarios, use `--no-agentic` flag to fall back to passive RAG.
...
- **Self-audit is VLM self-evaluation.** The audit flags are generated by the same model that wrote the report. This is useful for catching obvious inconsistencies but is not a substitute for human review.

---

## Future work (paper, not hackathon)

- **Agentic RAG ablation study:** Measure impact of query refinement iterations on retrieval quality and report grounding
